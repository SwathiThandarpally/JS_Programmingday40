//Read a Number 1, 10, 100, 1000, etc and display unit, ten, hundred,...
var number = ("10");//enter number between 1 to 10000000
if (number == 1) {console.log("One");} 
else if (number == 10){console.log("Ten");}
else if (number == 100) {console.log("Hundred");} 
else if (number == 1000){console.log("Thousand");}
else if (number == 10000) {console.log("Ten Thousand");} 
else if (number == 100000){console.log("One Lakh");}
else if (number == 1000000){console.log("Ten Lakhs");}
else if (number == 10000000){console.log("One Crore");}
else{console.log("Invalid input");}
